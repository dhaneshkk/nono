...
impl SecurityLists {
    ...
    pub fn macos_writable_paths(&self) -> Vec<String> {
        let mut paths = self.system_read_paths.macos.writable.clone();
        paths.extend(self.system_read_paths.macos.user_writable.iter().cloned());
        paths
    }
}
